...
def model(inputs, reuse=False, trainable=True, debug=False):
    # inputs.get_shape() = TensorShape([Dimension(228), Dimension(304), Dimension(74), Dimension(1)])
    if debug: print(inputs.get_shape())
    net = conv2d('coarse1', inputs, [11, 11, 3, 96], [96], [1, 4, 4, 1], padding='VALID', reuse=reuse, trainable=trainable)
    if debug: print(net.get_shape())
